...
class SmartAssistant:
    ...
    def __init__(self, use_gpu=False):
        ...
        self.special_gesture_cooldown = 2.5  # Increased cooldown to prevent accidental toggling
        self.special_gesture_hold_frames = 15  # Increased frames required for activation
    ...
    def process_frame(self, frame):
        ...
        if self.authorized_user:
            ...
            if new_gesture in self.special_gestures:
                ...
                # Only start counting detected frames when the gesture has been consistently detected for at least 3 consecutive frames
                # This helps with stability and prevents accidental activations
                if special_data["consecutive_frames"] >= 3:
                    special_data["detected_frames"] += 1
                else:
                    special_data["detected_frames"] = 0
                
                # Update hold status on display with more visual feedback
                hold_percent = min(100, (special_data["detected_frames"] / self.special_gesture_hold_frames) * 100)
                
                # Show hold progress with better visibility
                hold_color = (0, 255, 255) if new_gesture == "Draw" else (255, 0, 0)
                cv2.putText(display_frame, f"Hold {new_gesture}: {hold_percent:.0f}%", (10, 120), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, hold_color, 2)
                
                # Add progress bar for better visual feedback
                progress_bar_x = 250
                progress_bar_width = 200
                progress_bar_height = 20
                progress_filled = int((progress_bar_width * hold_percent) / 100)
                
                # Draw progress bar background
                cv2.rectangle(display_frame, (progress_bar_x, 110), 
                             (progress_bar_x + progress_bar_width, 110 + progress_bar_height), 
                             (100, 100, 100), -1)
                
                # Draw progress bar fill
                if progress_filled > 0:
                    cv2.rectangle(display_frame, (progress_bar_x, 110), 
                                 (progress_bar_x + progress_filled, 110 + progress_bar_height), 
                                 hold_color, -1)
                
                # Print debugging info for gesture
                if new_gesture == "Draw" or new_gesture == "OCR":
                    finger_debug = f"{new_gesture} detection: frames={special_data['detected_frames']}/{self.special_gesture_hold_frames}, consecutive={special_data['consecutive_frames']}"
                    cv2.putText(display_frame, finger_debug, (10, 150), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.6, hold_color, 1)
                    
                    # Add extra-visible activation indicator if close to activation
                    if hold_percent > 50:
                        radius = int(hold_percent / 2)
                        cv2.circle(display_frame, (display_frame.shape[1] - 100, 100), radius, hold_color, -1)
                ...
                # Also make sure we've had enough consecutive frames to avoid accidental activation
                if special_data["detected_frames"] >= self.special_gesture_hold_frames and special_data["consecutive_frames"] >= 5:
        ...
        if self.ocr_mode:
            ...
            # Process OCR on the ROI only every 10 frames for better performance
        ...
        if self.frame_count == 1:
            ...
            self.fps = 0.8 * self.fps + 0.2 * (1.0 / process_time)
